# flake.nix (extrait pertinent)
{
  ...
  let
    ...
    myOverlays = [
      ...
      (final: prev: {                          # <- Brave unstable (exemple)
        brave-unstable =
  ...
  system = "x86_64-linux";
  modules = [
    # Module inline pour appliquer les overlays avant le reste de la config
    ({ config, pkgs, ...}: {
      nixpkgs.overlays = [
  ...
  specialArgs = {
    ...
    homeConfigurations = {
      ...
      "${serviceUser}@test" = home-manager.lib.homeManagerConfiguration {
        inherit pkgs; extraSpecialArgs = { inherit serviceUser; };
